...
export const InteractiveMenuAI: React.FC = () => {
  ...
  const [hiddenProductId, setHiddenProductId] = useState<number | null>(null);
  ...
  const addToCart = (product: Product, event?: React.MouseEvent) => {
    ...
    if (event) {
      ...
      // Ocultar el producto original durante la animación
      setHiddenProductId(product.id);
      
      // Restaurar el producto después de la animación
      setTimeout(() => {
        setFlyingProduct(null);
        setHiddenProductId(null);
      }, 800);
    }
    ...
    <div className="min-h-screen overflow-hidden relative">
      ...
      >
        <div className="max-w-7xl mx-auto flex justify-between items-center">
          ...
          {/* Contenedor del carrito con posición relativa */}
          <div className="relative">
            ...
            {/* Panel del carrito posicionado absolutamente respecto al ícono */}
            <AnimatePresence>
              {cart.length > 0 && (
                <motion.div
                  ...
                  className="absolute right-0 top-12 bg-white rounded-2xl shadow-2xl p-4 min-w-[280px] max-w-[320px] z-50"
                >
                            ...
                            >
                              {products.map((product, productIndex) => (
                                <motion.div
                                  ...
                                  initial={{ opacity: 0, scale: 0.8 }}
                                  animate={{ 
                                    opacity: hiddenProductId === product.id ? 0 : 1, 
                                    scale: hiddenProductId === product.id ? 0.5 : 1 
                                  }}
                                  whileInView={{ opacity: hiddenProductId === product.id ? 0 : 1, scale: 1 }}
                                  transition={{ delay: productIndex * 0.05 }}
                                  whileHover={{ scale: hiddenProductId === product.id ? 0.5 : 1.05 }}
